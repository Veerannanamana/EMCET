...
branch = st.selectbox("🔹 Select Branch", ["Select an option"] + list(df['branch'].unique()))
caste = st.selectbox("🔹 Select Caste", ["Select an option"] + list(df['caste'].unique()))
gender = st.selectbox("🔹 Select Gender", ["Select an option"] + list(df['gender'].unique()))
rank = st.number_input("🔢 Enter Your Rank",  step=1)

# Ensure the user selects valid inputs before predicting
if st.button("🚀 Predict Top 5 Colleges"):
    if branch == "Select an option" or caste == "Select an option" or gender == "Select an option":
        st.warning("⚠️ Please select all fields before proceeding.")
    elif rank >= 200000:
        st.warning("⚠️ The entered rank is out of boundary. Please enter a valid rank (≤ 200000).")
    elif rank <=0:
        st.warning("⚠️ The entered rank is invalid. Please enter a valid rank (Eg:1 or 2).")
    else:
